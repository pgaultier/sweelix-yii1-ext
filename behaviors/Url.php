...
class Url extends \CActiveRecordBehavior {
	...
	public function deleteUrl($event) {
		...
		EntityUrl::model()->deleteAllByAttributes(array('urlElementType' => $urlElementType, 'urlElementId' => $urlElementId));
	}
	/**
	...
	public function transliterateUrl($event) {
		...
		$replaceRegex = array('-', '-', '.');
		$this->getOwner()->{$urlProperty} = trim(preg_replace($searchRegex, $replaceRegex, strtolower($this->getOwner()->{$urlProperty})), ' -');
		$dirname = pathinfo($this->getOwner()->{$urlProperty}, PATHINFO_DIRNAME);
		$this->getOwner()->{$urlProperty} = (($dirname == '.')?'':$dirname.'/').pathinfo($this->getOwner()->{$urlProperty}, PATHINFO_FILENAME);
