 ...
 * @property \sweelix\yii1\ext\entities\Meta[]   $metas
 * @property \sweelix\yii1\ext\entities\Tag[]    $tags
 * @property \sweelix\yii1\ext\entities\Author   $author
 * @property \sweelix\yii1\ext\entities\Language $language
 * @property \sweelix\yii1\ext\entities\Node     $node
 * @property \sweelix\yii1\ext\entities\Template $template
 *
...
class Content extends ActiveRecordContent
{
    ...
    public function checkSignature()
    {
        ...
        if ($dbSignature != $this->getContentSignature()) {
            $this->addError('contentSignature', Yii::t('sweelix', 'Content has been updated by {firstname} {lastname}',
                array(
                    '{firstname}' => $currentContent->author->authorFirstname,
                    '{lastname}' => $currentContent->author->authorLastname
                )));
        }
    }
    ...
    public function getContentElements($asArray = true)
    {
        if ($this->contentElements === null) {
            $this->contentElements = \CJSON::decode($this->contentData, $asArray);
        }
    ...
    {
        try {
            Yii::trace('Move content ' . $this->contentId . ' ' . $where . ' contentId : ' . $targetContentId,
                'sweelix.yii1.ext.entities');
    ...
    public function checkDate($attribute, $params)
    {
        if ((CPropertyValue::ensureBoolean($this->contentStartDateActive) === true)
            ...
            $startDate = CDateTimeParser::parse($this->contentStartDate,
                Yii::app()->getLocale()->getDateFormat('short'));
    ...
    public function beforeSave()
    {
        if (CPropertyValue::ensureBoolean($this->contentStartDateActive) === false) {
            ...
            $this->contentStartDate = Yii::app()->getLocale()->dateFormatter->format('yyyy-MM-dd',
                CDateTimeParser::parse($this->contentStartDate, Yii::app()->getLocale()->getDateFormat('short')));
        }
        if (CPropertyValue::ensureBoolean($this->contentEndDateActive) === false) {
            $this->contentEndDate = null;
        } else {
            if (is_string($this->contentEndDate) === true) {
                $this->contentEndDate = Yii::app()->getLocale()->dateFormatter->format('yyyy-MM-dd',
                    CDateTimeParser::parse($this->contentEndDate, Yii::app()->getLocale()->getDateFormat('short')));
            }
        }
    ...
    private function transcodeDates()
    {
        if (($this->contentStartDate !== null) && (is_string($this->contentStartDate) === true)) {
            $this->contentStartDate = Yii::app()->getLocale()->dateFormatter->format(Yii::app()->getLocale()->getDateFormat('short'),
                CDateTimeParser::parse($this->contentStartDate, 'yyyy-MM-dd HH:mm:ss'));
        }
        if (($this->contentEndDate !== null) && (is_string($this->contentEndDate) === true)) {
            $this->contentEndDate = Yii::app()->getLocale()->dateFormatter->format(Yii::app()->getLocale()->getDateFormat('short'),
                CDateTimeParser::parse($this->contentEndDate, 'yyyy-MM-dd HH:mm:ss'));
        }
    }
    ...
    {
        if (isset($this->hasTags[$tagId]) === false) {
            ...
            if ($result > 0) {
                $this->hasTags[$tagId] = true;
            } else {
                $this->hasTags[$tagId] = false;
            }
        }
    ...
    {
        if (isset($this->hasGroups[$groupId]) === false) {
            $criteriaBuilder = new CriteriaBuilder('content');
            $criteriaBuilder->filterBy('contentId', $this->contentId);
            $criteriaBuilder->filterBy('groupId', $groupId);
            $result = $criteriaBuilder->count();
            if ($result > 0) {
                $this->hasGroups[$groupId] = true;
            } else {
                $this->hasGroups[$groupId] = false;
            }
        }
        return $this->hasGroups[$groupId];
    }
