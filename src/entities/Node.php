...
class Node extends ActiveRecordNode
{
    ...
    public function checkSignature()
    {
        ...
        if ($dbSignature != $this->getNodeSignature()) {
            $this->addError('nodeSignature', Yii::t('sweelix', 'Node has been updated by {firstname} {lastname}',
                array(
                    '{firstname}' => $currentNode->author->authorFirstname,
                    '{lastname}' => $currentNode->author->authorLastname
                )));
        }
    }
    ...
    public function getNodeElements($asArray = true)
    {
        if ($this->nodeElements === null) {
            $this->nodeElements = CJSON::decode($this->nodeData, $asArray);
        }
    ...
    {
        try {
            Yii::trace('Move node ' . $this->nodeId . ' ' . $where . ' nodeId : ' . $targetNodeId,
    ...
    public function getParent()
    {
        if ($this->parent === null) {
            ...
            $this->parent = Node::model()->find($criteria);
            if ($this->parent === null) {
                $this->parent = false;
            }
        }
